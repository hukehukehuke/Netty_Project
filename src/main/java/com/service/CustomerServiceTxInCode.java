...
public class CustomerServiceTxInCode {
    ...
    public Customer create(Customer customer) {
        ...
        try {
            ...
            jmsTemplate.convertAndSend("customer : name",customer.getUsername());
    ...
    @JmsListener(destination = "customer jsm1: name")
    public Customer createByLisener(Customer customer) {
        LOG.info("CustomerService In Code create customer:{}", customer.getUsername());
        if (customer.getId() != null) {
            throw new RuntimeException("用户已经存在");
        }
